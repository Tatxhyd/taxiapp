...
class CreateDefenderPermissionRoleTable extends Migration
{
    ...
    public function up()
    {
        Schema::create(config('defender.permission_role_table', 'permission_role'), function (Blueprint $table) {
            $table->integer(config('defender.permission_key', 'permission_id'))->index('fk_per_role_per_id');
            $table->foreign(config('defender.permission_key', 'permission_id'),'fk_per_role_per_id')->references('id')
                  ->on(config('defender.permission_table', 'permissions'))
                  ->onDelete('cascade');

            $table->integer(config('defender.role_key', 'role_id'))->index('fk_per_role_role_id');
            $table->foreign(config('defender.role_key', 'role_id'),'fk_per_role_role_id')->references('id')
